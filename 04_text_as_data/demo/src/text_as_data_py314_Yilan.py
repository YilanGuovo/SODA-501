...
# ============================================================
# 7. In-Class Extension: Multinomial Outcome (movie genres)
# ============================================================

# (A) Create multinomial labels (map genre strings to integers)
# Mapping:
genre_to_label = {"action": 0, "comedy": 1, "drama": 2, "horror": 3, "other": 4}

# Make sure genre values are lowercase
df["genre_clean"] = df["true_topic"].astype(str).str.lower().str.strip()

# Map to integer labels
df["y_multiclass"] = df["genre_clean"].map(genre_to_label)

# Optional: if some rows are unmapped, put them into "other"
# (This avoids NaNs if your dataset has extra genres)
df.loc[df["y_multiclass"].isna(), "y_multiclass"] = genre_to_label["other"]
df["y_multiclass"] = df["y_multiclass"].astype(int)


# (B) Train/test split (use stratify to keep class balance)
X_train, X_test, y_train, y_test = train_test_split(
    doc_vectors,
    df["y_multiclass"].values,
    test_size=0.25,
    random_state=123,
...
# (C) Multinomial model (Multinomial Logistic Regression)
...
# (D) Evaluate: accuracy + confusion matrix
acc = accuracy_score(y_test, y_pred)
cm = confusion_matrix(y_test, y_pred, labels=[0, 1, 2, 3, 4])

print("--- Embedding MULTICLASS classification (Multinomial Logistic Regression) ---")
print("Accuracy:", round(acc, 4))
print("Confusion Matrix:\n", cm)

# Save metrics
metrics_multi = pd.DataFrame(
    {"model": ["sbert_multinomial_logreg"], "accuracy": [acc]}
)
metrics_multi.to_csv("outputs/week_embedding_multiclass_metrics.csv", index=False)

# Save confusion matrix as CSV too (often helpful)
cm_df = pd.DataFrame(
    cm,
    index=["true_action", "true_comedy", "true_drama", "true_horror", "true_other"],
    columns=["pred_action", "pred_comedy", "pred_drama", "pred_horror", "pred_other"],
)
cm_df.to_csv("outputs/week_embedding_multiclass_confusion_matrix.csv", index=True)

# Plot confusion matrix (matplotlib only, no seaborn)
disp = ConfusionMatrixDisplay(
    confusion_matrix=cm,
    display_labels=["Action", "Comedy", "Drama", "Horror", "Other"]
)

plt.figure(figsize=(8, 7))
disp.plot(values_format="d", cmap="Blues", ax=plt.gca(), colorbar=False)
plt.title("Confusion Matrix (SBERT + Multinomial Logistic Regression)")
plt.tight_layout()
plt.savefig("figures/embedding_multiclass_confusion_matrix.png", dpi=200)
plt.show()
